function [LL, prior, transmat, mu, Sigma, mixmat, exp_num_trans, actions] = ...
     mhmm_em(data,action_index, prior, transmat, mu, Sigma, mixmat, varargin);
...
% Own edit: identify actions
actions = unique(action_index);
nact = size(actions,1);
...
while (num_iter <= max_iter) & ~converged
  ...
  for i=1:nact
      ...
      [loglik1, exp_num_trans1, exp_num_visits11, postmix1, m1, ip1, op1] = ...
      ess_mhmm(prior, transmat{i}, mixmat, mu, Sigma, data(action_index==actions(i)));
  
  loglik=loglik + loglik1;
  exp_num_trans(:,:,i) = exp_num_trans1;
  exp_num_visits1 = exp_num_visits1 + exp_num_visits11;
  postmix = postmix + postmix1;
  m = m + m1;
  op = op + op1;
  ip = ip + ip1;
  end
  ...
  if adj_trans 
    for i=1:nact
      transmat{i} = mk_stochastic(exp_num_trans(:,:,i));
    end
  end
  if adj_mix
